...
import { isPrimOp, CExp, PrimOp, VarDecl, Binding, unparseClassExp } from './L3-ast';
...
export const isClassVal = (x: any): x is ClassVal => x.tag === "Class";
...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | ClassVal | Object;
...
export const valueToString = (val: Value): string =>
    ...
    isClassVal(val) ? ClassValToString(val) :
    isObjectVal(val) ? ObjectValToString(val) :
    isPrimOp(val) ? val.op :
